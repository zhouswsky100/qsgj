...
app.directive('ionicCityPicker', ['$ionicPopup', '$timeout','CityPickerService','$ionicScrollDelegate','$ionicModal', function ($ionicPopup, $timeout,CityPickerService, $ionicScrollDelegate,$ionicModal) {
  ...
  return {
    ...
    link: function (scope, element, attrs) {
        ...
        if (angular.isDefined(so.areaDataId) && so.areaDataId.length > 1) {
          vm.areaDataId = so.areaDataId
          vm.areaData = vm.defaultAreaData.join(vm.tag)
        } else {
          vm.defaultAreaData =  ['北京','东城区']
          vm.areaDataId = ['110000', '110100']
        ...
        vm.returnOk = function(){
          if(vm.city && vm.city.sub && vm.city.sub.length > 0) {
            vm.areaDataId = [vm.province.id, vm.city.id, vm.country.id];
            vm.areaData = vm.province.name + vm.tag +  vm.city.name + vm.tag + vm.country.name;
          } else {
            vm.areaDataId = [vm.province.id, vm.city.id];
        ...
        vm.initAreaData = function(AreaDataIds) {
          if (AreaDataIds[0]) { // 初始化省
            for (var i = 0; i < vm.AreaService.length; i++)
            {
              if (AreaDataIds[0] === vm.AreaService[i].id){
          ...
          if (AreaDataIds[1] && vm.province && vm.province.sub) { // 初始化市
            for (var i = 0; i < vm.province.sub.length; i++)
            {
              if (AreaDataIds[1] === vm.province.sub[i].id){
          ...
          if (AreaDataIds[2] && vm.city && vm.city.sub) { // 初始化区
            for (var i = 0; i < vm.city.sub.length; i++)
            {
              if (AreaDataIds[2] === vm.city.sub[i].id){
        ...
        if (vm.watchChange) {
          scope.$watch('options.areaDataId', function(newVal,oldVal){
              if (newVal !== oldVal && isArray(newVal) && newVal.length > 1 && newVal !== vm.areaDataId) {
                if (vm.isCreated) {
                  vm.initAreaData(newVal)
                }else {
                  vm.areaDataId = newVal
                }
                //vm.areaData = newVal.join(vm.tag)
              }
          })

          scope.$watch('options.defaultAreaData', function(newVal,oldVal){
              if (newVal !== oldVal && isArray(newVal) && newVal.length > 1 && newVal !== vm.areaDataId) {
